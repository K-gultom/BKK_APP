...
<div class="container mt-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h4 class="mb-0">Data Permohonan Surat Layak Terbang</h4>
        <div>
            <a href="{{ url('/surat-layak-terbang-dashboard') }}" class="btn btn-outline-secondary me-2">
    ...
    <div class="row mb-3">
        <div class="col-md-4">
            <form method="GET" action="{{ url('/surat-layak-terbang-dashboard') }}">
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-hover align-middle">
            ...
            <tbody>
                @forelse ($dokumen as $index => $item)
                    <tr>
                        ...
                        <td class="text-center">
                            ...
                            {{-- Edit Status --}}
                            <button type="button" class="btn btn-sm btn-warning" data-bs-toggle="modal" data-bs-target="#editStatusModal{{ $item->id }}">
                        ...
                        <div class="modal-dialog modal-lg">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    ...
                                    <ul>
                                        <li>Nama Pasien: {{ $item->nama_pasien }}</li>
                                        <li>Tanggal Lahir: {{ $item->tgl_lahir }}</li>
                                        <li>Usia: {{ $item->usia }}</li>
                                        <li>Jenis Kelamin: {{ $item->jenis_kelamin }}</li>
                                        <li>Nomor Identitas: {{ $item->nomor_identitas }}</li>
                                        <li>Tanggal Pemeriksaan: {{ $item->tgl_pemeriksaan }}</li>
                                    </ul>
                                    <h5 class="fw-bold">Rencana Penerbangan</h5>
                                    <ul>
                                        <li>Tanggal Penerbangan: {{ $item->tgl_penerbangan }}</li>
                                        <li>Kota Asal: {{ $item->kota_asal }}</li>
                                        <li>Kota Tujuan: {{ $item->kota_tujuan }}</li>
                                    </ul>
                                </div>
                    ...
                    <div class="modal fade" id="editStatusModal{{ $item->id }}" tabindex="-1">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form action="{{ route('surat-layak-terbang-dashboard.update', $item->id) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                        <div class="mb-3">
                ...
                @empty
                    <tr>
                        <td colspan="5" class="text-center">Tidak ada pengajuan Surat Layak Terbang</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
